...
class AIService:
    ...
    async def generate_financial_suggestions(
        ...
        try:
            ...
            for suggestion in suggestions:
                ...
                tasks.append(self.collection.insert_one(insight.dict(by_alias=True)))

            results = await asyncio.gather(*tasks)
            inserted_ids = [res.inserted_id for res in results]

            # fetch docs in one query instead of many
            docs = await self.collection.find({"_id": {"$in": inserted_ids}}).to_list(
                length=len(inserted_ids)
            )
            return [self._format_insight_response(doc) for doc in docs]

        except Exception:
            return await self._generate_fallback_suggestions(
                clerk_user_id, user_profile, goals
            )

    async def calculate_savings_plan(
        self, clerk_user_id: str, goal_id: str
    ) -> SavingsPlanResponse:
        ...
        Suggest the best investment option about 2-3 with expected returns and risk, 
        but respond in ONE short, clear sentence only.
        ...
        return SavingsPlanResponse(
            ...
            expected_returns=self._calculate_expected_returns(
                monthly_required, goal.duration
            ),
        )

    async def handle_ai_query(
        self, clerk_user_id: str, query: str, context: str = None
    ) -> AIInsightResponse:
        ...
        Provide a concise, helpful financial response. If a calculation is needed, clearly show the math step by step. If offering advice, make it practical and actionable in short response.
        Include:
        Expected Returns – give a short 2 line estimate or projection.
        Risk Factors – highlight key risks in short 2 lines.
        """
    ...
    async def get_user_insights(
        self, clerk_user_id: str, limit: int = 10
    ) -> List[AIInsightResponse]:
    ...
    def _prepare_user_context(self, user_profile: Dict[str, Any], goals: List) -> str:
        ...
        surplus = user_profile.get("monthly_income", 0) - user_profile.get(
            "monthly_expenses", 0
        )
    ...
    async def _generate_fallback_suggestions(
        self, clerk_user_id: str, user_profile: Dict, goals: List
    ) -> List[AIInsightResponse]:
        ...
        for suggestion in suggestions:
            ...
            result = await self.collection.insert_one(insight.dict(by_alias=True))
            created_insight = await self.collection.find_one(
    ...
    def _get_default_investment_suggestion(self, category: str) -> str:
        ...
        return suggestions.get(
            category, "Consult a financial advisor for personalized advice"
        )
    ...
    def _calculate_expected_returns(self, monthly_amount: float, duration: int) -> str:
        ...
        future_value = monthly_amount * (
            ((1 + monthly_rate) ** duration - 1) / monthly_rate
        )
    ...
    def _format_insight_response(self, insight_doc: dict) -> AIInsightResponse:
        ...
        return AIInsightResponse(
            ...
            content=insight_doc.get("content"),
            actionable=insight_doc.get("actionable"),
